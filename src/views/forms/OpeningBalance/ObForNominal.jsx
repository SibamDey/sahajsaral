...
const ObForNominal = () => {
    ...
    const onSearch = () => {
        if (!selectedMonth) {
            ...
            getNominalAccountForOBList(userData?.CORE_LGD, currentFinancialYear, selectedMonth).then((response) => {
    ...
    const onSave = () => {
        const isAnyFieldEmpty = data.some(
            (row) => String(row.openingBalance).trim() === ""
        );
        ...
        if (isAnyFieldEmpty) {
            ...
            addInsertNominalAccountOB(userData?.CORE_LGD, currentFinancialYear, selectedMonth, userData?.USER_INDEX, data,
                (r) => {
                    console.log(r, "dd");
                    if (r.status == 0) {
                        setData([]);
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-2 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">Nominal Account Code-Wise Opening Balance
                </legend>
                        ...
                        <tbody>
                            {data.map((row, index) => (
                                ...
                                >
                                    ...
                                    <td className="border border-gray-300 text-sm p-1">{row.groupName}</td>
