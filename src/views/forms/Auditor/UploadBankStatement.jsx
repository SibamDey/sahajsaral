...
const UploadBankStatement = () => {
    ...
    const getLgdCode = () => {
        if (userData?.USER_LEVEL === "DIST") return userData?.DIST_LGD;
        if (userData?.USER_LEVEL === "BLOCK") return userData?.BLOCK_LGD;
        if (userData?.USER_LEVEL === "GP") return userData?.GP_LGD;
        return userData?.CORE_LGD || 0;
    };

    const getStatementUrl = (path) => {
        if (!path) return "#";
    ...
    const fetchBankStatementList = async (finYearValue = "0", accountCodeValue = "0") => {
        ...
        try {
            ...
            const response = await axios.get(
                "https://javaapi.wbpms.in/api/AuditFile/GetBankStatement",
                {
                    params: {
                        lgdCode: userData?.CORE_LGD,
                        finYear: finYearValue || "0",
                        accountCode: accountCodeValue || "0",
                    },
                }
            );

            setBankStatementList(Array.isArray(response?.data) ? response.data : []);
        } catch (error) {
            console.error("Error fetching bank statement list:", error);
            setBankStatementList([]);
            toast.error("Failed to load bank statement list");
        } finally {
    ...
    useEffect(() => {
        const lgdCode = getLgdCode();

        if (!lgdCode) return;

        getRealAccList(lgdCode)
            .then((result) => {
                const filteredBankAccounts = Array.isArray(result?.data)
                    ? result.data.filter(
                        (item) =>
                            String(item?.accountCode) !== "900000201" &&
                            String(item?.accountCode) !== "900000291"
                    )
                    : [];

                setRealAccList(filteredBankAccounts);
            })
            .catch((error) => {
                console.error("Error fetching bank account list:", error);
                toast.error("Failed to load bank account list");
            });
    ...
    const onFinancialYearChange = (e) => {
        ...
        if (isUpdateMode) {
            ...
            const fileInput = document.getElementById("bankStatementPdf");
    ...
    const onUploadBankStatement = async () => {
        if (!userData?.CORE_LGD) {
        ...
        try {
            ...
            const response = await axios.post(
                "https://javaapi.wbpms.in/api/AuditFile/UploadBankStatement",
                payload
            );

            if (response?.data?.statusCode === 1) {
                toast.error(response?.data?.message || "Failed to upload bank statement");
                return;
            }

            toast.success(
                response?.data?.message ||
                (isUpdateMode ? "Bank statement updated successfully" : "Bank statement uploaded successfully")
            );

            const uploadedFinYear = currentFinancialYear;
            const uploadedAccountCode = accountCode;

            resetUploadFields();
            fetchBankStatementList(uploadedFinYear, uploadedAccountCode);
        } catch (error) {
    ...
    const onSearchBankStatementList = () => {
        if (!currentFinancialYear) {
            toast.error("Please select financial year");
            return;
        }
    ...
    const onUpdateClick = (item) => {
        ...
        setCurrentFinancialYear(item?.finYear || "");
                    ...
                    <>
                        ...
                        {isUpdateMode && (
                            <div className="mb-3 rounded-md border border-amber-300 bg-amber-50 px-4 py-2 text-sm text-amber-800">
                                Update mode enabled for Account Code:{" "}
                                <span className="font-semibold">{updateRowData?.accountCode}</span>,
                                Financial Year:{" "}
                                ...
                                <div className="flex items-end gap-4 bg-white border border-gray-200 rounded-md p-4 shadow-sm">
                                    ...
                                    <div className="flex flex-col w-2/5">
                                        ...
                                        >
                                            ...
                                            {realAccList?.map((item, index) => (
                                                <option key={index} value={item?.accountCode}>
                                    ...
                                    <div className="flex items-end gap-2">
                                        ...
                                        <button
                                            ...
                                            onClick={onSearchBankStatementList}
                                        >
                        ...
                        <table className="min-w-full text-sm text-left border-collapse">
                            ...
                            <tbody>
                                {bankStatementList.length > 0 ? (
                                    bankStatementList.map((item, index) => (
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 px-2 py-2 text-center">
                                                {item?.statementDocs ? (
                                                    <a
                                                        href={getStatementUrl(item.statementDocs)}
                                                        target="_blank"
                                                        rel="noreferrer"
