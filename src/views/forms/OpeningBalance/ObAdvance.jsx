...
import { getOBAdvance, addInsertOBAdvance } from "../../../Service/OpeningBalance/OpeningBalance";
...
const ObAdvance = () => {
    ...
    const onSearch = () => {
        if (!selectedMonth) {
            ...
            getOBAdvance(userData?.CORE_LGD, currentFinancialYear, selectedMonth).then((response) => {
    ...
    const onSave = () => {
        ...
        if (isAnyFieldEmpty) {
            ...
            addInsertOBAdvance(userData?.CORE_LGD, currentFinancialYear, selectedMonth, userData?.USER_INDEX, data,
                (r) => {
                    console.log(r, "dd");
                    if (r.status == 0) {
                        setData([]);
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-2 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">OB – Advance
                </legend>
