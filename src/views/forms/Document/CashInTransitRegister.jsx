...
const CashInTransitRegister = () => {
    ...
    const { data: FinBranchList } = useQuery({
        ...
        queryFn: async () => {
            const data = await fetch.get("/FinBranch/Get?lgdCode=" + (userData?.USER_LEVEL === "GP" ? userData?.GP_LGD : userData?.USER_LEVEL === "BLOCK" ? userData?.BLOCK_LGD : userData?.DIST_LGD) + "&mode=" + 0);
    ...
    const { mutate: addPed, isPending: addPending, data: result, } = useMutation({
        mutationFn: (newTodo) => {
            return fetch.get(`/Register/CashInTransit?lgdCode=${userData?.CORE_LGD}&reportType=${mode.current.value}&frmDate=${fromDate.current.value}&toDate=${toDate.current.value}`);
        },
    ...
    const list = [
        ...
        {
            header: "En-Cash Voucher Id",
            accessorKey: "encashVoucherId",
            headclass: "cursor-pointer",
        },
        {
            header: "En-Cash On Date",
            accessorKey: "encashOn",
            headclass: "cursor-pointer",
        },
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-2 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">Cash-in-Transit Register</legend>
                ...
                <div className=" flex justify-between items-center h-12">
                    <div className="flex items-center space-x-0">
                        ...
                        <button
                            ...
                            onClick={() => exportToExcel(data, "Cash_In_Transit_Register")}
...
export default CashInTransitRegister;
