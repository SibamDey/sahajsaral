...
import { getOBLiquidCash, addInsertOBLiquidCash } from "../../../Service/OpeningBalance/OpeningBalance";


const ObForLiquidCash = () => {
    ...
    const onSearch = () => {
        if (!selectedMonth) {
            ...
            getOBLiquidCash(userData?.CORE_LGD, currentFinancialYear, selectedMonth).then((response) => {
    ...
    const onSave = () => {
        ...
        if (isAnyFieldEmpty) {
            ...
            addInsertOBLiquidCash(userData?.CORE_LGD, currentFinancialYear, selectedMonth, userData?.USER_INDEX, data,
                (r) => {
                    console.log(r, "dd");
                    if (r.status == 0) {
                        setData([]);
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-2 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">Headwise OB – Liquid Cash
                </legend>
...
export default ObForLiquidCash;
