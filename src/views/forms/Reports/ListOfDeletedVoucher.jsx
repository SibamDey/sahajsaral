...
import { getListDeleteVoucher } from "../../../Service/Reports/ReportsService";
import * as XLSX from "xlsx";


const ListOfDeleteVoucher = () => {
    ...
    const { data: nominalAccList } = useQuery({
        ...
        queryFn: async () => {
            const data = await fetch.get("/ReportViewer/DeletedVoucherList?lgdCode=" + (userData?.CORE_LGD) + "&frmDate=" + (fromDate) + "&toDate=" + (toDate));
            return data?.data;
        },
    });
    ...
    const onSearch = () => {
        if (!fromDate) {
            ...
            getListDeleteVoucher(userData?.CORE_LGD, fromDate, toDate)
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-1 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">List of Deleted Vouchers</legend>
...
export default ListOfDeleteVoucher;
