...
const ActivityWiseExpenditure = () => {
    ...
    const { mutate: addPed, isPending: addPending, data: result, } = useMutation({
        mutationFn: (newTodo) => {
            return fetch.get(`/ReportViewer/ActivityWiseExpenditure?lgdCode=${userData?.CORE_LGD}&frmDate=${fromDate.current.value}&toDate=${toDate.current.value}&schemeId=${mode.current.value ? mode.current.value : 0}`);
        },
    ...
    return (
        <>
            ...
            <div className="bg-white rounded-lg p-2 flex flex-col flex-grow" style={{ marginTop: "-40px" }}>
                <legend className="text-lg font-semibold text-cyan-700">Activity Wise Expenditure</legend>
...
export default ActivityWiseExpenditure;
